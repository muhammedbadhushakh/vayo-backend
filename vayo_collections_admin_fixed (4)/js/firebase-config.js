...
db.settings({
  ...
  // Buffering proxies/antivirus that interfere with long-polling often have
  // their own shorter idle timeout than Firestore's default 30s "hanging
  // GET". When that happens, the proxy silently kills the connection before
  // Firestore's own timeout fires, so the SDK just waits forever for a
  // response that's already been dropped. Asking Firestore to close and
  // retry sooner (25s) gives it a chance to recover within that window
  // instead of waiting on a connection nothing intends to answer.
